...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    if (!hasColumn(sceneTable, "roomModelPublicId")) {
      await queryInterface.addColumn("showroom_scenes", "roomModelPublicId", {
        type: Sequelize.STRING,
        allowNull: true,
      });
    }

    if (!hasColumn(sceneTable, "roomModelMimeType")) {
      await queryInterface.addColumn("showroom_scenes", "roomModelMimeType", {
        type: Sequelize.STRING,
        allowNull: true,
      });
    }

    if (!hasColumn(sceneTable, "roomModelFileName")) {
      await queryInterface.addColumn("showroom_scenes", "roomModelFileName", {
        type: Sequelize.STRING,
        allowNull: true,
      });
    }

    if (!hasColumn(sceneTable, "roomModelSizeBytes")) {
      await queryInterface.addColumn("showroom_scenes", "roomModelSizeBytes", {
  ...
  async down(queryInterface) {
    ...
    const maybeRemove = async (name) => {
      if (hasColumn(sceneTable, name)) {
        await queryInterface.removeColumn("showroom_scenes", name);
      }
    };

    await maybeRemove("roomModelSizeBytes");
    await maybeRemove("roomModelFileName");
    await maybeRemove("roomModelMimeType");
    await maybeRemove("roomModelPublicId");
  },
};
